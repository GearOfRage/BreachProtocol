...
public class ResultPanelController : MonoBehaviour
{
    ...
    [Header("Exploit")] [SerializeField] private GameObject exploitHolder;
    ...
    [SerializeField] private GameObject overLimitExploit;
    ...
    {
        sb.Clear();
        sb.Append("//root folder").AppendLine().Append("//access request").AppendLine().Append("//access granted")
            .AppendLine();
    }
    ...
    public void AddExploit()
    {
        ...
        if (GameMaster._instance.activeExploits.Count < GameMaster._instance.GetComponent<PlayerManager>().limitExploits)
        {
            GameMaster._instance.exploitsCounterText.text = 
                "Memory " + (GameMaster._instance.activeExploits.Count + 1) + "/" + GameMaster._instance.GetComponent<PlayerManager>()
                                                                .limitExploits;
        ...
        {
            // Give extra score
            PlayerManager._instance.score += 500;

            Exploit newExploit = overLimitExploit.GetComponent<Exploit>();
            exploitIconComp.sprite = newExploit.exploitIcon;
            exploitNameComp.text = newExploit.GetComponent<Exploit>().exploitName;
            exploitDescComp.text = newExploit.GetComponent<Exploit>().exploitDesc;

            exploitDeployButton.SetActive(false);
            exploitFragmentButton.SetActive(false);
        }
    }
    ...
    public void DeployButtonClick()
    {
        ...
        GameObject newExploit =
            Instantiate(GameMaster._instance.exploitsPrefabs[randExploitIndex],
                GameMaster._instance.exploitsHolder.transform);
